...
import { comfyuiTools, handleComfyUITool } from './comfyui-tools.js';
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return { tools: [...tools, ...universalTools, ...vipTools, ...comfyuiTools] };
});
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'generate_image':
      case 'check_comfyui_status':
      case 'list_comfyui_workflows': {
        const result = await handleComfyUITool(name, args || {});
        return result;
      }
